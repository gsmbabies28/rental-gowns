...
export const SideTabInput = ({name,type,label,value,placeHolder}) => {
  const {search, handleQueries,handleNavigate,location} = useContext(ShopPageContext);
  ...
  switch(type) {
    case "checkbox":
      ...
      if(search.getAll(name).includes(value))
        isChecked  = true;
    break;
    case "radio":
      styles = 'form-radio';
          ...
          <input
            ...
            onChange={(e) =>{
              ...
              switch(type){
                ...
                case "checkbox":
                  handleQueries(e.target.name, e.target.value, e.target.checked);
                  break;
